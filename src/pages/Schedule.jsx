...
const ScheduleCard = ({ a, onClick, index }) => {
  ...
  useEffect(() => {
    ...
    if (a.episode) {
      ...
      const animeSlug = a.slug ? a.slug.replace(/-episode-\d+$/, '') : a.id;
      fetch(`/anime/stream/anime/${animeSlug}`)
        .then(res => res.json())
        .then(d => {
          const epList = d.data?.episode_list || d.data?.episodes || [];
          if (mounted && epList.length > 0) {
            const firstEp = epList[0];
            setEp(typeof firstEp === 'string' ? firstEp.match(/-episode-(\d+)$/)?.[1] : firstEp.index);
          }
        }).catch(() => null);
    }
    ...
    <div className={`flex items-stretch gap-4 md:gap-6 relative group w-full mb-4 transition-all duration-700 ease-out ${isVisible ? 'opacity-100 blur-none translate-y-0' : 'opacity-0 blur-xl translate-y-8'}`}>
      ...
      <div className="flex-1 py-4 w-full min-w-0">
        <div onClick={onClick} className="relative rounded-xl flex p-3 md:p-4 gap-4 md:gap-5 cursor-pointer transition-all active:scale-[0.98] bg-[#16161a] border border-white/5 hover:border-[#F6CF80]/30 group/card overflow-hidden shadow-xl">
          <img src={a.poster} referrerPolicy="no-referrer" className="w-20 md:w-24 aspect-[3/4] object-cover rounded-md shadow-2xl shrink-0 group-hover/card:scale-105 transition-transform duration-500 relative z-10" />
          <div className="flex flex-col flex-1 min-w-0 overflow-hidden relative z-10">
            ...
            <div className="flex flex-col gap-1.5 text-[10px] md:text-xs text-white/60">
              <div className="flex items-start gap-2">
                ...
                <span className="flex-1 text-white/80 font-medium line-clamp-1">{a.genre || '-'}</span>
              </div>
              <div className="flex items-start gap-2">
...
const Schedule = () => {
  ...
  useEffect(() => {
    ...
    const fetchSchedule = async () => {
      setIsLoading(true);
      try {
        // ✅ Fetch dari API route, data udah di-enrich Jikan di server
        const res = await fetch('/api/schedule').then(r => r.json());
        const latestList = Array.isArray(res.data) ? res.data : (Array.isArray(res) ? res : []);
        const grouped = { ALL: latestList.map(a => ({ ...a, status: "ONGOING" })) };
        if (isMounted) setSchedule(grouped);
      } catch (e) {
      } finally {
  ...
  const getAnimeList = () => schedule["ALL"] || [];
        ...
        <div className="flex flex-col">
          {isLoading ? <ScheduleSkeleton /> : (
            getAnimeList().length > 0 ? getAnimeList().map((a, index) => (
              <ScheduleCard
                key={`${a.slug || index}-${index}`}
                a={a}
                index={index}
                onClick={() => navigate(`/anime/${a.slug}`, { state: { latestEp: true } })}
              />
            )) : (
